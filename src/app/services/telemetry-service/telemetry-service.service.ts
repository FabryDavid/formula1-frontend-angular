...
export class TelemetryServiceService {
  ...
  constructor(private http: HttpClient, private serverResponseConverter: ServerResponseConverter) {
  }
    ...
    return this.http.get<any>(`${environment.apiUrl}/all-car-data/${gp}/${session}/2022/${lap}/${drivers.join(",")}`).pipe(
      map((data => {
          ...
          data.forEach((telemetry: any) => {
            const carData: ITelemetryCarData = ServerResponseConverter.telemetryCarData(telemetry.carData)
